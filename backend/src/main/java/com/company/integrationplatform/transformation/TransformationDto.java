...
public class TransformationDto {
        ...
        @Getter
        @Setter
        @Schema(name = "TransformationRuleCreateRequest", description = "Payload for creating a new transformation rule")
        public static class CreateRequest {

                @NotBlank(message = "Rule name is required")
                @Size(max = 255, message = "Name must not exceed 255 characters")
                @Schema(description = "Unique rule name within the data source scope", example = "Normalize Customer Name", requiredMode = Schema.RequiredMode.REQUIRED)
                private String name;

                @Size(max = 500)
                @Schema(description = "Optional description of what this rule does", example = "Trims whitespace from the customer_name field")
                private String description;

                @Schema(description = "Data source UUID this rule applies to. Null = global rule.", example = "7c9e6679-7425-40de-944b-e07fc1f90ae7")
                private UUID dataSourceId;

                @NotNull(message = "Transformation type is required")
                @Schema(description = "Type of transformation to apply", example = "TRIM", allowableValues = {
                                "DIRECT_MAPPING", "UPPERCASE", "LOWERCASE", "TRIM",
                                "CONCAT", "DEFAULT_VALUE", "DATE_FORMAT" }, requiredMode = Schema.RequiredMode.REQUIRED)
                ...
                @Schema(description = "Field name in the source record to read from", example = "customer_name")
                ...
                @Schema(description = "Field name in the output record to write to. "
                                + "If null, result overwrites sourceField.", example = "customerName")
                ...
                @Schema(description = "Execution order — lower numbers run first", example = "10")
        ...
        @Getter
        @Setter
        @Schema(name = "TransformationRuleUpdateRequest", description = "Payload for updating a transformation rule. All fields optional.")
        public static class UpdateRequest {
                ...
                @Schema(description = "Updated transformation type", allowableValues = { "DIRECT_MAPPING", "UPPERCASE",
                                "LOWERCASE", "TRIM",
        ...
        @Getter
        @Builder
        @Schema(name = "TransformationRuleResponse", description = "Transformation rule details returned by the API")
        public static class Response {
                ...
                @Schema(description = "Scoped data source UUID (null = global)", example = "7c9e6679-7425-40de-944b-e07fc1f90ae7")
                private UUID dataSourceId;
        ...
        @Getter
        @Builder
        @Schema(name = "TransformationApplyResult", description = "Result of applying transformation rules to an ingestion job")
        public static class ApplyResult {

                @Schema(description = "Ingestion job ID the rules were applied to", example = "3fa85f64-5717-4562-b3fc-2c963f66afa6")
                private UUID jobId;
